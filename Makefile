...
RUNTIME_BIN := $(BUILD_DIR)/amlogic-transformers-block-runner
REFERENCE_ADAPTER := $(BUILD_DIR)/libamlogic_transformers_reference_adapter.so
...
$(RUNTIME_BIN): runtime/src/main.cpp runtime/src/block_runner.cpp runtime/src/block_runner.h runtime/include/amlogic_transformers/adapter_abi.h | $(BUILD_DIR)
...
$(REFERENCE_ADAPTER): runtime/adapters/reference_copy_adapter.cpp runtime/include/amlogic_transformers/adapter_abi.h | $(BUILD_DIR)
...
fixture: build-runtime
	PYTHONPATH=src $(PYTHON) scripts/make_runtime_fixture.py --output $(FIXTURE_DIR) --adapter ../libamlogic_transformers_reference_adapter.so
