...
4. Build a board-specific adapter implementing `gemma_aml_init`, `gemma_aml_run`, and `gemma_aml_shutdown`.
5. Run `gemma-npu-block-runner` and pass drift thresholds.
...
The adapter ABI in `runtime/include/gemma_npu/adapter_abi.h` is the stable
...
They should copy or map NPU output into the provided `GemmaTensor.data` memory
